...
/**
 * Integration tests for {@link JdbcSessionRepositoryAutoConfiguration} against PostgreSQL
 * via Testcontainers JDBC URL.
 *
 * @author Christian Tzolov
 */
class JdbcSessionRepositoryPostgresqlAutoConfigurationIT {

	private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
		.withConfiguration(AutoConfigurations.of(JdbcSessionRepositoryAutoConfiguration.class,
				JdbcTemplateAutoConfiguration.class, DataSourceAutoConfiguration.class))
		.withPropertyValues("spring.datasource.url=jdbc:tc:postgresql:17:///sessiontest",
	...
	void saveAndFindSession() {
		this.contextRunner.run(context -> {
			...
			Session session = Session.builder().id(sessionId).userId("user-pg").build();
	...
	void appendAndFindEvents() {
		this.contextRunner.run(context -> {
			var repo = context.getBean(JdbcSessionRepository.class);
			String sessionId = UUID.randomUUID().toString();
			repo.save(Session.builder().id(sessionId).userId("user-pg-events").build());

			SessionEvent event1 = SessionEvent.builder()
				.id(UUID.randomUUID().toString())
				.sessionId(sessionId)
				.timestamp(Instant.ofEpochSecond(1_700_000_000L))
				.message(new UserMessage("Hello from PostgreSQL"))
				.build();
			...
			assertThat(events.get(0).getMessage().getText()).isEqualTo("Hello from PostgreSQL");
	...
	void deleteSessionCascadesToEvents() {
		this.contextRunner.run(context -> {
			var repo = context.getBean(JdbcSessionRepository.class);
			String sessionId = UUID.randomUUID().toString();
			repo.save(Session.builder().id(sessionId).userId("user-pg-delete").build());
	...
	void findByUserId() {
		this.contextRunner.run(context -> {
			var repo = context.getBean(JdbcSessionRepository.class);
			String userId = "user-pg-multi-" + UUID.randomUUID();
